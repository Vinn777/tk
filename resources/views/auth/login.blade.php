    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-end mt-4">
            <x-primary-button class="ms-3 w-full justify-center py-3 text-lg" style="background: #2E7D32 !important;">
                {{ __('Masuk Portal Guru') }}
            </x-primary-button>
        </div>

        <div class="mt-6 text-center space-y-2">
            <a href="{{ route('login.parent') }}" class="block text-sm text-gray-500 hover:text-green-700">Login sebagai Orang Tua? Klik di sini</a>
            <a href="{{ route('register') }}" class="block text-sm font-bold text-green-700 hover:underline">Belum punya akun? Buat Akun Baru</a>
        </div>
    </form>
</x-guest-layout>
